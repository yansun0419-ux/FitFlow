...
const ManagerDashboard = () => {
  ...
  const loadStudents = async () => {
    ...
    setLoading(true);
    try {
      const data = await listStudentsRequest(token, 1, 20);
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 relative">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 px-4">
          <div>
                ...
                {loading ? (
                  ...
                  filteredStudents.map((student) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <Badge className="bg-emerald-50 text-emerald-700 border border-emerald-100 px-2 py-0.5 text-[10px] font-black uppercase">
                          Active
                        </Badge>
                      </td>
                      <td className="px-6 py-4 text-right">
                        <Button
                          ...
                          className="text-indigo-600 hover:bg-indigo-50 font-bold text-xs rounded-xl px-4 py-2 float-right"
                        >
      ...
      >
        <div className="p-8">
            ...
            {historyLoading ? (
              ...
              history.map((enrollment) => {
                ...
                return (
                  <div
                    key={enrollment.id}
                    className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100 group hover:border-indigo-200 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">
                          {course.weekday} • {course.start_time} -{" "}
                          {course.end_time}
                        </p>
                      </div>
                    </div>
          ...
          <div className="mt-8">
            <Button
              variant="secondary"
              className="w-full py-3 rounded-2xl font-bold uppercase tracking-widest text-xs"
